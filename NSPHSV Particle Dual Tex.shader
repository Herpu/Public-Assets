...
Shader ".Herpu/NSPHSV Particle 2 Dual Tex" {
Properties {
		[Header(Texture)]
	[HDR]_TintColor ("Tint Color", Color) = (0.5,0.5,0.5,0.5)
	_MainTex ("Particle Texture", 2D) = "white" {}
	_SecondTex ("Texture Mask", 2D) = "white" {}
		[Header(Scrolling)]
	...
	x2scrollspeed ("Mask X Scroll", Float) = 0
	y2scrollspeed ("Mask Y Scroll", Float) = 0
	ofs ("Mask Tile and Offset", Vector)=(1,1,0,0)
	...
	SubShader {
		Pass {
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float u2= i.texcoord.x*ofs.x+ofs.z+_Time*x2scrollspeed;
				float v2= i.texcoord.y*ofs.y+ofs.w+_Time*y2scrollspeed;

				float2 maskUV = float2(u2,v2);
				fixed4 col1 = tex2D(_MainTex, uvscroll);
				fixed4 mask = tex2D(_SecondTex, maskUV);
				...
				fixed3 col = 2.0f * i.color.rgb * _TintColor.rgb * OUTPUT.rgb * col1.a * i.color.a * _TintColor.a *mask.rgb *mask.a;
				
				
				float4 nigga = float4(col,1);
				if(_Blend==10){
				col = 2.0f * i.color.rgb * _TintColor.rgb * OUTPUT.rgb * mask.rgb;
				nigga = float4(col,col1.a * i.color.a * _TintColor.a * mask.a);
				}
				return nigga;
			}
			ENDCG
